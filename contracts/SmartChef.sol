...
contract SmartChef is Ownable {
    ...
    // The block number when SATOSHI mining ends.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 satoshiReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accSatoshiPerShare = accSatoshiPerShare.add(satoshiReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accSatoshiPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // Stake SATOSHI tokens to TheBushV1
    ...
    // Withdraw SATOSHI tokens from STAKING.
